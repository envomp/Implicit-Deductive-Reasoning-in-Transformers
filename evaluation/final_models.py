...
def filter_scaling_scores(raw_scores):
    ...
    for key, vals in raw_scores.items():
        ...
        if has_scaling and not has_layers:
            continue
        new_key = list(key)
        if not has_scaling:
    ...
    for (eval_domain, tags), methods in pairs.items():
        if 'cot' in methods and 'direct' in methods:
            ...
            for depth_limit, depth_label in [(6, '<=6'), (12, '<=12')]:
                ...
                if std_diff == 0.0:
                    ni_p_val = 0.0 if mean_diff < margin else 1.0
                else:
                    t_stat, ni_p_val = stats.ttest_1samp(diffs, popmean=margin, alternative='less')
                is_closed = ni_p_val < 0.05
                std_err = stats.sem(diffs) if std_diff > 0 else 0
                t_crit = stats.t.ppf(0.95, df=n-1) if n > 1 else 0
                implied_upper_bound = mean_diff + (t_crit * std_err)
                results.append({
                    'eval_domain': eval_domain.upper(),
                    'layers': layer_count,
                    'depth_group': depth_label,
                    'mean_diff': mean_diff,
                    'upper_bound_95': implied_upper_bound,
                    'ni_p_value': ni_p_val,
...
def plot_scaling_curves(data_30, data_60, max_depth=12, eval="lp", compare="pred", cls=""):
    plt.style.use('seaborn-v0_8-whitegrid')
    plt.rcParams['pdf.fonttype'] = 42
    plt.rcParams['ps.fonttype'] = 42
    ...
    def get_model_info(key):
        ...
        if "scaling" not in key:
            return "$L = 8$"
        for item in key:
            if isinstance(item, str) and item.startswith("layers="):
                ls = item.split('=')[1]
    ...
    plt.tight_layout()
    plt.savefig(f"final_models_scaling_curves_{cls}.pdf", format='pdf', bbox_inches='tight')
    plt.close(fig)
...
filtered_scores_30 = filter_scaling_scores(raw_scores_baseline_deep_30 | raw_scores_scaling_deep_30)
filtered_scores_60 = filter_scaling_scores(raw_scores_baseline_deep_60 | raw_scores_scaling_deep_60)
scaling_gaps_30 = evaluate_gap_closure(filtered_scores_30)
scaling_gaps_60 = evaluate_gap_closure(filtered_scores_60)
...
# plot_scaling_curves(direct_30_scaling, cot_30_scaling, compare="mode", cls="p30")
direct_60_scaling = {x:y for x, y in (raw_scores_baseline_deep_60 | raw_scores_scaling_deep_60).items() if x[1] == False}
cot_60_scaling = {x:y for x, y in (raw_scores_baseline_deep_60 | raw_scores_scaling_deep_60).items() if x[1] == True}
# plot_scaling_curves(direct_60_scaling, cot_60_scaling, compare="mode", cls="p60")
